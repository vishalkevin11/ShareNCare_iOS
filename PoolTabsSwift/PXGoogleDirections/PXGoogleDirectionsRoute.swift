...
open class PXGoogleDirectionsRoute: NSObject {
	...
	open var legs: [PXGoogleDirectionsRouteLeg] = [PXGoogleDirectionsRouteLeg]()
	...
	open var waypointsOrder: [Int] = [Int]()
	...
	open var overviewPolyline: String?
	/// Viewport bounding box of the `overviewPolyline`
	open var bounds: GMSCoordinateBounds?
	/// Copyrights text to be displayed for this route
	open var copyrights: String?
	/// Array of warnings to be displayed when showing these directions
	open var warnings: [String] = [String]()
	...
	open var totalDistance: CLLocationDistance {
		get {
...
//     //   let redYellow = GMSStrokeStyle.gradientFromColor(UIColor.redColor(), toColor:UIColor.yellowColor())
//		if approximate {
...
//	}
    ...
    open func drawOnMap(_ map: GMSMapView, withIndex: Int, approximate: Bool = false, strokeColor: UIColor = UIColor.red, strokeWidth: Float = 2.0) -> [GMSPolyline] {
        var polylines = [GMSPolyline]()
        //   let redYellow = GMSStrokeStyle.gradientFromColor(UIColor.redColor(), toColor:UIColor.yellowColor())
        if approximate {
            let polyline = GMSPolyline(path: path)
            ...
            for l in legs {
                ...
                for s in l.steps {
                    ...
                    //var patha : GMSPath = GMSPath.init(fromEncodedPath: s.polyline!)!
                    let polyline = GMSPolyline(path: s.path)
	...
	*/
	open func drawOriginMarkerOnMap(_ map: GMSMapView, title: String = "", color: UIColor = UIColor.red, opacity: Float = 1.0, flat: Bool = false) -> GMSMarker? {
	...
	- returns: The resulting `GMSMarker` object that was drawn to the map
	*/
	open func drawDestinationMarkerOnMap(_ map: GMSMapView, title: String = "", color: UIColor = UIColor.red, opacity: Float = 1.0, flat: Bool = false) -> GMSMarker? {
		var marker: GMSMarker?
		if let p = path {
			if p.count() > 1 {
