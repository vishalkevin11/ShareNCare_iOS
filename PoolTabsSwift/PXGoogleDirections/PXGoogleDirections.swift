...
open class PXGoogleDirections: NSObject {
	...
	open var optimizeWaypoints: Bool?
	/// If set to `true`, specifies that the API may provide more than one route alternative in the response
	open var alternatives: Bool?
	/// Indicates that the calculated routes should avoid the indicated features
	open var featuresToAvoid = Set<PXGoogleDirectionsFeature>()
	/// Specifies the unit system to use when displaying results
	open var units: PXGoogleDirectionsUnit?
	///Specifies the desired time of departure
	open var departureTime: PXTime?
	/// Specifies the desired time of arrival (for transit directions only)
	open var arrivalTime: PXTime?
	/// Specifies one or more preferred modes of transit
	open var transitModes = Set<PXGoogleDirectionsTransitMode>()
	/// Specifies preferences for transit routes
	open var transitRoutingPreference: PXGoogleDirectionsTransitRoutingPreference?
	...
	open var delegate: PXGoogleDirectionsDelegate?
	...
	*/
	open var directionsAPIRequestURL: URL? {
	...
	*/
	open func calculateDirections(_ completion: @escaping PXGoogleDirectionsRequestCompletionBlock)
	{
	...
	*/
	open func openInGoogleMaps(center: CLLocationCoordinate2D?, mapMode: PXGoogleMapsMode?, view: Set<PXGoogleMapsView>?, zoom: UInt?, callbackURL: URL?, callbackName: String?, fallbackToAppleMaps: Bool = true) -> Bool {
		// Ensure both origin and destination are set
		if let f = from, let t = to {
			// Ensure there is actually something specified for both origin and destination addresses
			if f.isSpecified() && t.isSpecified() {
	...
	internal class func buildGoogleMapsURL(params: [String], callbackURL: URL?, callbackName: String?) -> URL? {
		...
		if PXGoogleDirections.canOpenInGoogleMaps {
			...
			if let cbURL = callbackURL, let cbn = callbackName {
                ...
                p.append("x-success=\(cbURL.absoluteString.addingPercentEncoding( withAllowedCharacters: .urlQueryAllowed)!)")
